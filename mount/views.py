...
def withdraw(request):
    ...
    Transaction.objects.create(
        user=account.user, account=account, amount=amount,
        transaction_type="withdraw", status="successful",
    )
    ...
    if request.method == "POST":
        ...
        if tx_type == "deposit":
            ...
            with db_transaction.atomic():
                ...
                receiver.save()
                Transaction.objects.create(
                    user=request.user, account=sender, amount=amount,
                    transaction_type="transfer", status="successful",
                )
...
def call_clubkonnect_data_api(network, phone, plan_id, request_id):
    ...
    params = {
        ...
        "DataPlan": plan_id, "MobileNumber": phone,
        "RequestID": request_id, "CallBackURL": "",
    }
...
def buy_data(request):
    if request.method == "POST":
        ...
        try:
            ...
            logger.error(f"buy_data owner error: {e}")
        ...
        if success_flag:
            DataPurchase.objects.create(
                user=request.user, network=network, phone_number=phone,
                amount=amount, status="successful", reference=request_id,
            )
        ...
        DataPurchase.objects.create(
            user=request.user, network=network, phone_number=phone,
            amount=amount, status="failed", reference=request_id,
        )
        return redirect("buy_data")
...
def cancel_foreign_number(request, order_id):
    ...
    try:
        ...
        logger.info(
            f"5SIM cancel → HTTP {response.status_code} | {response.text[:300]}"
        )

        if response.status_code != 200:
            messages.error(
                request,
                "Unable to cancel number. It may already be expired or cancelled."
            )
            return redirect("buy_foreign_number")
        ...
        with db_transaction.atomic():
            ...
            # Lock records
            fn = ForeignNumber.objects.select_for_update().get(
                pk=foreign_number.pk
            )
            ...
            customer_account = Account.objects.select_for_update().get(
                user=request.user
            )

            owner_account = Account.objects.select_for_update().get(
                pk=get_owner_account().pk
            )
            ...
            logger.info(
                f"Refunding ₦{refund_amount} | "
                f"Customer Before={customer_account.balance} "
                f"Owner Before={owner_account.balance}"
            )

            # Refund customer
            customer_account.balance += refund_amount
            customer_account.save()

            # Remove from owner
            if owner_account.pk != customer_account.pk:
                owner_account.balance -= refund_amount
                owner_account.save()

            logger.info(
                f"Customer After={customer_account.balance} "
                f"Owner After={owner_account.balance}"
            )

        messages.success(
            request,
            f"Number cancelled successfully. ₦{refund_amount:,.0f} refunded to your wallet."
        )
...
# =========================
# FOREIGN NUMBERS — STEADYSIM HELPERS
# =========================
...
# =========================
# FOREIGN NUMBERS — STEADYSIM VIEWS
# =========================
...
def buy_foreign_number_steadysim(request):
    ...
    numbers = ForeignNumber.objects.filter(
        user=request.user, provider="steadysim"
    ).order_by("-created_at")
...
def cancel_foreign_number_steadysim(request, order_id):
    try:
        foreign_number = ForeignNumber.objects.get(
            order_id=order_id, user=request.user, provider="steadysim"
        )
    except ForeignNumber.DoesNotExist:
        messages.error(request, "Number not found.")
        return redirect("buy_foreign_number_steadysim")
    ...
    if foreign_number.status == "FINISHED":
        messages.error(request, "This number has already received an SMS and cannot be cancelled.")
        return redirect("buy_foreign_number_steadysim")
    ...
    if ok:
        ...
        logger.error(f"SteadySim cancel failed for order {order_id}: {msg}")
        if foreign_number.status == "PENDING":
            logger.warning(f"Force-cancelling SteadySim order {order_id}. Provider: {msg}. Manual review needed.")
            cancel_succeeded = True
        else:
            messages.error(request, f"Could not cancel: {msg}. Contact support if a refund is owed.")
...
def buy_electricity(request):
    ...
    if request.method == "GET" and request.GET.get("verify_meter"):
        ...
        return JsonResponse({
            "success":       ok,
            "customer_name": result if ok else "",
...
def buy_cable_tv(request):
    ...
    if request.method == "GET" and request.GET.get("verify_card"):
        ok, result = verify_smartcard(
            request.GET.get("cable_tv", ""),
            request.GET.get("smartcard_no", ""),
        )
        return JsonResponse({
            "success":       ok,
            "customer_name": result if ok else "",
    ...
    if request.method == "POST":
        ...
        if len(phone) < 11:
            ...
            return redirect("buy_cable_tv")
        package_info = next(
            (p for p in CABLE_TV_PACKAGES.get(cable_tv, []) if p["code"] == package_code), None
        )
        if not package_info:
